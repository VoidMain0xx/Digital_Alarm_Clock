...
const resumeBtn = document.querySelector('#res-btn');
...
// The data is not deleted when the browser is closed, and are available for future sessions. (W3School)


//if the user exited the web page
if(!localStorage.getItem('userExited')){
    localStorage.setItem('userExited' , 'false');
}else{
    // check wheathe client have any previous alar set
...
//play Sound
function playSound(){
    if( (localStorage.getItem('userExited') == 'xxx')
    || ((localStorage.getItem('wantToPlay') == 'yes'))){
        ringTone.play();
    }
    ringTone.loop = true;
}
...
function hideWelcomeScerrn(){
    // hiding Screen 
    //d - none is display none class
    welcomeBackScreen.className = 'd-none';
    //displaying alarm time indicator
    alarmTimeIndicator.textContent = "alarm Time is set to : " + localStorage.getItem('alarmTime');
    // avoiding dom Exception
    localStorage("userExited" , 'xxx');
    localStorage.setItem('wantToPlay' , 'yes');
}

function alarmTimeFunc() {
    let date = new Date();
    ...
    //suppose the the time is 16 so 16 - 12 = 4 12 hour format
    if (h > 11) {
        h = h - 12;
        ampm = "PM"
    }

    //if the value is 0 then set it to 12
    h = h == 0 ? h = 12 : h;
    //adding zero in single digits no
    h = h < 10 ? "0" + h : h;
    m = m < 10 ? "0" + m : m;
    s = s < 10 ? "0" + s : s;

    //updating time as per the system
...
// alarmTimeIndicator();
setInterval(alarmTimeFunc, 1000);
...
// Resume Button
resumeBtn.addEventListener('click', hideWelcomeScerrn);
